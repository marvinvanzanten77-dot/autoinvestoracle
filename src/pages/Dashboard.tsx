...
function DashboardHeader({ onScan, lastScan }: { onScan: () => void; lastScan: string }) {
  return (
    <div className="glass rounded-2xl p-4 md:p-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <div className="mt-1 flex items-center gap-3 text-sm text-slate-600">
          ...
          <span>Laatste scan: {lastScan}</span>
        </div>
      </div>
      <button
        type="button"
...
function WalletCard({ amount }: { amount: number }) {
  ...
  return (
    <Card title="Wallet (mock)" subtitle="Beschikbaar saldo">
...
function PlatformsCard() {
  return (
    <Card title="Platforms om te verkennen" subtitle="Plus- en minpunten op een rij">
...
export function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-5 md:gap-6">
      <DashboardHeader onScan={handleScan} lastScan={lastScan} />
